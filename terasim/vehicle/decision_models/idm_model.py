...
class IDMModel(HighwayBaseDecisionModel):
    ...
    def load_parameters(self, IDM_parameters, MOBIL_parameters):
        ...
        self.COMFORT_ACC_MAX = IDM_parameters["COMFORT_ACC_MAX"] if "COMFORT_ACC_MAX" in IDM_parameters else COMFORT_ACC_MAX
        self.COMFORT_ACC_MIN = IDM_parameters["COMFORT_ACC_MIN"] if "COMFORT_ACC_MIN" in IDM_parameters else COMFORT_ACC_MIN
        self.DISTANCE_WANTED = IDM_parameters["DISTANCE_WANTED"] if "DISTANCE_WANTED" in IDM_parameters else DISTANCE_WANTED
        self.TIME_WANTED = IDM_parameters["TIME_WANTED"] if "TIME_WANTED" in IDM_parameters else TIME_WANTED
        self.DESIRED_VELOCITY = IDM_parameters["DESIRED_VELOCITY"] if "DESIRED_VELOCITY" in IDM_parameters else DESIRED_VELOCITY
        self.DELTA = IDM_parameters["DELTA"] if "DELTA" in IDM_parameters else DELTA
        self.stochastic_IDM_resolution = IDM_parameters["stochastic_IDM_resolution"] if "stochastic_IDM_resolution" in IDM_parameters else stochastic_IDM_resolution
        self.stochastic_IDM_prob_threshold = IDM_parameters["stochastic_IDM_prob_threshold"] if "stochastic_IDM_prob_threshold" in IDM_parameters else stochastic_IDM_prob_threshold
        ...
        self.POLITENESS = MOBIL_parameters["POLITENESS"] if "POLITENESS" in MOBIL_parameters else POLITENESS
        self.LANE_CHANGE_MIN_ACC_GAIN = MOBIL_parameters["LANE_CHANGE_MIN_ACC_GAIN"] if "LANE_CHANGE_MIN_ACC_GAIN" in MOBIL_parameters else LANE_CHANGE_MIN_ACC_GAIN
        self.LANE_CHANGE_MAX_BRAKING_IMPOSED = MOBIL_parameters["LANE_CHANGE_MAX_BRAKING_IMPOSED"] if "LANE_CHANGE_MAX_BRAKING_IMPOSED" in MOBIL_parameters else LANE_CHANGE_MAX_BRAKING_IMPOSED
    ...
    def IDM_acceleration(self, ego_vehicle=None, front_vehicle=None, distance=None):
        ...
        if front_vehicle is not None:
            ...
            d = max(1e-5, r)
            acceleration -= a0 * \
                np.power(self.desired_gap(ego_vehicle, front_vehicle) / d, 2)
        return acceleration
    ...
    def mobil_gain(self, lane_index, observation):
        ...
        new_following_distance = new_following["distance"] + new_preceding["distance"] + 2*mingap + self.LENGTH if (new_following and new_preceding) else 99999
        new_following_a = self.IDM_acceleration(ego_vehicle=new_following, front_vehicle=new_preceding, distance=new_following_distance)
        new_following_pred_a = self.IDM_acceleration(ego_vehicle=new_following, front_vehicle=ego, distance=new_following["distance"] if new_following else 99999)
        ...
        old_following_a = self.IDM_acceleration(ego_vehicle=old_following, front_vehicle=ego, distance=old_following["distance"] if old_following else 99999)
        old_following_pred_a_distance = old_following["distance"] + old_preceding["distance"] + 2*mingap + self.LENGTH if (old_following and old_preceding) else 99999
        old_following_pred_a = self.IDM_acceleration(ego_vehicle=old_following, front_vehicle=old_preceding, distance=old_following_pred_a_distance)
