    ...
    def _check_longitudinal_safety(obs, pdf_array, lateral_result=None, CAV_flag=False):
        ...
        for i in range(len(pdf_array)-1, -1, -1):
            ...
            if f_veh_info is not None:
                ...
                self_v_2, f_v_2 = max(ego_info["velocity"] + acc, HighwayBaseDecisionModel.v_low), max((f_veh_info["velocity"] + HighwayBaseDecisionModel.acc_low), HighwayBaseDecisionModel.v_low)                
                dist_r = (self_v_2**2 - HighwayBaseDecisionModel.v_low **2)/(2*abs(HighwayBaseDecisionModel.acc_low))
                dist_f = (f_v_2**2 - HighwayBaseDecisionModel.v_low **2)/(2*abs(HighwayBaseDecisionModel.acc_low)) + HighwayBaseDecisionModel.v_low * (f_v_2 - self_v_2) / HighwayBaseDecisionModel.acc_low
        ...
        for lane_index, nearby_veh in zip(lane_change_dir,nearby_vehs):
            if pdf_array[lane_index] != 0:
                ...
                if f_veh is not None:
                    ...
                    r = f_veh["distance"]
                    dis_change = rr*HighwayBaseDecisionModel.ACTION_STEP + 0.5*HighwayBaseDecisionModel.acc_low*(HighwayBaseDecisionModel.ACTION_STEP**2)
                    r_1 = r + dis_change # 1s
                    rr_1 = rr + HighwayBaseDecisionModel.acc_low*HighwayBaseDecisionModel.ACTION_STEP
        
                    if r_1 <= safety_buffer or r <= safety_buffer:
                        pdf_array[lane_index] = 0             
                    elif rr_1 < 0:
                        self_v_2, f_v_2 = max(CAV_info["velocity"], HighwayBaseDecisionModel.v_low), max((f_veh["velocity"] + HighwayBaseDecisionModel.acc_low), HighwayBaseDecisionModel.v_low)                
                        dist_r = (self_v_2**2 - HighwayBaseDecisionModel.v_low **2)/(2*abs(HighwayBaseDecisionModel.acc_low))
                        dist_f = (f_v_2**2 - HighwayBaseDecisionModel.v_low **2)/(2*abs(HighwayBaseDecisionModel.acc_low)) + HighwayBaseDecisionModel.v_low * (f_v_2 - self_v_2) / HighwayBaseDecisionModel.acc_low
                ...
                if r_veh is not None:
                    ...
                    r = r_veh["distance"]
                    dis_change = rr*HighwayBaseDecisionModel.ACTION_STEP - 0.5*HighwayBaseDecisionModel.acc_high*(HighwayBaseDecisionModel.ACTION_STEP**2)
                    r_1 = r + dis_change
                    rr_1 = rr - HighwayBaseDecisionModel.acc_high*HighwayBaseDecisionModel.ACTION_STEP
                    if r_1 <= safety_buffer or r <= safety_buffer:
                        pdf_array[lane_index] = 0                        
                    elif rr_1 < 0:
                        self_v_2, r_v_2 = min(CAV_info["velocity"], HighwayBaseDecisionModel.v_high), min((r_veh["velocity"] + HighwayBaseDecisionModel.acc_high), HighwayBaseDecisionModel.v_high)                
                        dist_r = (r_v_2**2 - HighwayBaseDecisionModel.v_low **2)/(2*abs(HighwayBaseDecisionModel.acc_low))
                        dist_f = (self_v_2**2 - HighwayBaseDecisionModel.v_low **2)/(2*abs(HighwayBaseDecisionModel.acc_low)) + HighwayBaseDecisionModel.v_low * (-r_v_2 + self_v_2) / HighwayBaseDecisionModel.acc_low
                        r_2 = r_1 - dist_r + dist_f 
                        if r_2 <= safety_buffer:
                            pdf_array[lane_index] = 0
