...
class Simulator(object):    
    ...
    def __init__(self,
        ...
        assert isinstance(additional_sumo_args, list) or isinstance(additional_sumo_args, str) or additional_sumo_args is None, "additional_sumo_args should be a list of strings or a string"
        self.additional_sumo_args = [additional_sumo_args] if isinstance(additional_sumo_args, str) else additional_sumo_args
    ...
    def start(self):
        ...
        """
        sumo_cmd = [self.sumo_binary, "-c", str(self.sumo_config_file_path), "--step-length", str(self.step_length)]
        if self.sumo_output_file_types is not None:
            ...
            if "fcd" in self.sumo_output_file_types:
                ...
                sumo_cmd += ["--fcd-output", filename, "--fcd-output.acceleration", "--device.fcd.explicit", "CAV", "--device.fcd.radius", "200"]
            ...
            if "tripinfo" in self.sumo_output_file_types:
                ...
                sumo_cmd += ["--tripinfo-output", filename, "--tripinfo-output.write-unfinished"]
        # log file
    ...
    def subscribe_vehicle_surrounding(vehID, max_obs_range=120):
        ...
        traci.vehicle.subscribeContext(vehID, tc.CMD_GET_VEHICLE_VARIABLE, max_obs_range, [tc.VAR_LENGTH, tc.VAR_POSITION, tc.VAR_SPEED, tc.VAR_LANE_INDEX, tc.VAR_ANGLE, tc.VAR_POSITION3D, tc.VAR_ROAD_ID, tc.VAR_ACCELERATION, tc.VAR_LANE_ID])
    ...
    def subscribe_vehicle_all_information(vehID, max_obs_range=120):
        ...
        """
        Simulator.subscribe_vehicle_ego(vehID)
        traci.vehicle.subscribeContext(vehID, tc.CMD_GET_VEHICLE_VARIABLE, max_obs_range, [tc.VAR_LENGTH, tc.VAR_POSITION, tc.VAR_SPEED, tc.VAR_LANE_INDEX, tc.VAR_ANGLE, tc.VAR_POSITION3D, tc.VAR_EDGES, tc.VAR_LANEPOSITION, tc.VAR_LANEPOSITION_LAT, tc.VAR_SPEED_LAT, tc.VAR_ROAD_ID, tc.VAR_ACCELERATION, tc.VAR_LANE_ID])
        traci.vehicle.addSubscriptionFilterLanes([-2,-1,0,1,2], noOpposite=True, downstreamDist=max_obs_range, upstreamDist=max_obs_range)
