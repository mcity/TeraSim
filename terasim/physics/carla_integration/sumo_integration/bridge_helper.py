...
class BridgeHelper(object):
    ...
    def _get_recommended_carla_blueprint_from_sumo_redis(sumo_actor_vclass_value):
        ...
        for blueprint in BridgeHelper.blueprint_library:
            if (
                blueprint.id in BridgeHelper._VTYPES
                and BridgeHelper._VTYPES[blueprint.id]["vClass"] == vclass
            ):
    ...
    def get_carla_blueprint(sumo_actor, sync_color=False):
        ...
        if type_id in [bp.id for bp in blueprint_library]:
            ...
            logging.debug(
                "[BridgeHelper] sumo vtype %s found in carla blueprints", type_id
            )
        else:
            ...
            if blueprint is not None:
                ...
                logging.error(
                    "sumo vtype %s not supported. No vehicle will be spawned in carla",
                    type_id,
                )
                return None
        ...
        if blueprint.has_attribute("color"):
            if sync_color:
                color = "{},{},{}".format(
                    sumo_actor.color[0], sumo_actor.color[1], sumo_actor.color[2]
                )
            else:
                color = random.choice(
    ...
    @staticmethod
    def get_carla_blueprint_from_sumo_redis(
        sumo_actor_type_id, sumo_actor_color, sumo_actor_vclass_value, sync_color=False
    ):
        """
        Returns an appropriate blueprint based on the received sumo actor.
        """
        ...
        if type_id in [bp.id for bp in blueprint_library]:
            ...
            blueprint = BridgeHelper._get_recommended_carla_blueprint_from_sumo_redis(
                sumo_actor_vclass_value
            )
            if blueprint is not None:
                tmp = 0
                # logging.warning(
        ...
        if blueprint.has_attribute("color"):
            if sync_color:
                color = "{},{},{}".format(
                    sumo_actor_color[0], sumo_actor_color[1], sumo_actor_color[2]
                )
            else:
                color = random.choice(
                    blueprint.get_attribute("color").recommended_values
    ...
    def get_carla_lights_state(current_carla_lights, sumo_lights):
        ...
        # Break.
        if bool(sumo_lights & SumoVehSignal.BRAKELIGHT) != bool(
            current_lights & carla.VehicleLightState.Brake
        ):
            current_lights ^= carla.VehicleLightState.Brake

        # Front (low beam).
        if bool(sumo_lights & SumoVehSignal.FRONTLIGHT) != bool(
            current_lights & carla.VehicleLightState.LowBeam
        ):
            current_lights ^= carla.VehicleLightState.LowBeam

        # Fog.
        if bool(sumo_lights & SumoVehSignal.FOGLIGHT) != bool(
            current_lights & carla.VehicleLightState.Fog
        ):
            current_lights ^= carla.VehicleLightState.Fog

        # High beam.
        if bool(sumo_lights & SumoVehSignal.HIGHBEAM) != bool(
            current_lights & carla.VehicleLightState.HighBeam
        ):
            current_lights ^= carla.VehicleLightState.HighBeam

        # Backdrive (reverse).
        if bool(sumo_lights & SumoVehSignal.BACKDRIVE) != bool(
    ...
    def get_sumo_lights_state(current_sumo_lights, carla_lights):
        ...
        # Blinker right.
        if bool(carla_lights & carla.VehicleLightState.RightBlinker) != bool(
            current_lights & SumoVehSignal.BLINKER_RIGHT
        ):
            current_lights ^= SumoVehSignal.BLINKER_RIGHT

        # Blinker left.
        if bool(carla_lights & carla.VehicleLightState.LeftBlinker) != bool(
        ...
        # Break.
        if bool(carla_lights & carla.VehicleLightState.Brake) != bool(
            current_lights & SumoVehSignal.BRAKELIGHT
        ):
            current_lights ^= SumoVehSignal.BRAKELIGHT

        # Front (low beam)
        if bool(carla_lights & carla.VehicleLightState.LowBeam) != bool(
            current_lights & SumoVehSignal.FRONTLIGHT
        ):
            current_lights ^= SumoVehSignal.FRONTLIGHT

        # Fog light.
        if bool(carla_lights & carla.VehicleLightState.Fog) != bool(
            current_lights & SumoVehSignal.FOGLIGHT
        ):
            current_lights ^= SumoVehSignal.FOGLIGHT

        # High beam ligth.
        if bool(carla_lights & carla.VehicleLightState.HighBeam) != bool(
            current_lights & SumoVehSignal.HIGHBEAM
        ):
            current_lights ^= SumoVehSignal.HIGHBEAM

        # Backdrive (reverse)
        if bool(carla_lights & carla.VehicleLightState.Reverse) != bool(
    ...
    def get_carla_traffic_light_state(sumo_tl_state):
        ...
        """
        if (
            sumo_tl_state == SumoSignalState.RED
            or sumo_tl_state == SumoSignalState.RED_YELLOW
        ):
            ...
            sumo_tl_state == SumoSignalState.GREEN
            or sumo_tl_state == SumoSignalState.GREEN_WITHOUT_PRIORITY
        ):
