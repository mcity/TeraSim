...
from terasim_cosmos.utils.wds_utils import write_to_tar, get_sample
from terasin_cosmos.utils.minimap_utils import simplify_minimap
from terasim_cosmos.utils.animation_utils import FreePoseAnimator, InterpType
...
def main(input_root, novel_pose_folder, dataset, clip_id):
    ...
    config_path = package_root / f'config/dataset_{dataset}.json'
    ...
    with server.gui.add_folder("Control"):
        ...
        gui_reset_to_first_camera_button = server.gui.add_button(
            "Reset to First Camera View",
            hint="Reset camera view to current frame's first camera position and orientation",
        )
    ...
    # >>> Setup gui_reset_to_first_camera_button
    @gui_reset_to_first_camera_button.on_click
    def _(event: viser.GuiEvent):
        client = event.client
        assert client is not None
        ...
        try:
            ...
            for client_id, client in server.get_clients().items():
                client.camera.position = current_camera_position_with_offset
                client.camera.wxyz = current_camera_wxyz

            client.add_notification(
                title="Camera View Reset",
                body=f"Camera view reset to frame {current_frame_idx} first camera position: {current_camera_position_with_offset}",
                loading=False,
                with_close_button=True,
                auto_close=3000,
                color='green',
            ...
            client.add_notification(
                title="Error",
                body=f"Failed to reset camera view: {str(e)}",
                loading=False,
                with_close_button=True,
                auto_close=3000,
                color='red',
