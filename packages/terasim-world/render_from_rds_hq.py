    ...
    for camera_name in settings['CAMERAS']:
        ...
        if camera_type == "pinhole":
            ...
            if os.path.exists(intrinsic_file):
                ...
                intrinsic_file = "assets/example/ftheta_intrinsic/2d23a1f4-c269-46aa-8e7d-1bb595d1e421_2445376400000_2445396400000.tar"
                if 'CAMERAS_TO_RDS_HQ' in settings:
                    camera_name_in_rds_hq = settings['CAMERAS_TO_RDS_HQ'][camera_name]    
                else:
                    camera_name_in_rds_hq = camera_name
                intrinsic_data = get_sample(intrinsic_file)
...
def render_sample_hdmap(
    ...
    """
    print(f"Processing {clip_id} {camera_type} from {input_root}...")
    ...
    for camera_name, camera_model in camera_name_to_camera_model.items():
        cprint(f"Processing minimap projection for {clip_id} {camera_name}...", 'green', attrs=['bold'])
        pose_all_frames = camera_name_to_camera_poses[camera_name]
        # camera_pose_initial_position = pose_all_frames[0][:3, 3]
        camera_pose_last_position = pose_all_frames[-1][:3, 3]
        ...
        for minimap_wds_file in minimap_wds_files:
            ...
            for point in minimap_data_wo_meta_info:
                ...
                distances = np.linalg.norm(point - camera_pose_last_position, axis=1)
        ...
        for frame_id in render_frame_ids:
            ...
            if frame_key in all_object_info:
                ...
                for obj, obj_information in frame_objects.items():
                    ...
                    distance = np.linalg.norm(obj_center - camera_pose_last_position)
