    ...
    def _(event: viser.GuiEvent):
        ...
        try:
            for client_id, client in server.get_clients().items():
                client.camera.position = initial_camera_position_with_offset
                client.camera.wxyz = initial_camera_wxyz
            
            client.add_notification(
                title="Camera View Reset",
                body=f"Camera view reset to first camera's initial position: {initial_camera_position_with_offset}",
                loading=False,
                with_close_button=True,
                auto_close=3000,
                color='green',
